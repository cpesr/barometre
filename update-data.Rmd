...
results <- results %>%
  ...
  mutate(categorie.grp = fct_recode(categorie,
    ...
    `BIATSS B` = "BIATSS catégorie B ou assimilé",
    `BIATSS C` = "BIATSS catégorie C ou assimilé")) %>%
...
## Sauvegarde 
save(results,conditions.factor,confiance.factor,etab.factor, file="results.RData")
...
resume <- function(bloc) {
  ...
  NNP <- df %>%
    mutate(Réponse = factor(case_match(str_sub(Réponse,1,1),
             "N" ~ "NCP",
             c("0","1","2","3") ~ "Négatif",
             c("4","5","6") ~ "Neutre",
             c("7","8","9","10") ~ "Positif"),
             levels = c("Négatif","Neutre","Positif","NCP"))) %>%
    summarise(nb.réponses = n(), .by=c(Question,Réponse)) %>%
    ...
    pivot_wider(names_from = Réponse, values_from = part)
